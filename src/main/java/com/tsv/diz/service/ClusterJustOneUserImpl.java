...
import com.tsv.diz.service.interfaces.ClusterJustOneUser;
@Service
public class ClusterJustOneUserImpl implements ClusterJustOneUser{
	...
	@Override
	public void clusterJustOneUserMethod(Long id) throws Exception{
		fileNumber++;
		//clusterValue = 0;
		...
		for (int i = 0; i < listaRezultate.size(); i++) {
			...
			// SCRIERE IN FISIER
			FileWriter writer = new FileWriter("output"+ fileNumber +".arff");
		...
		// ALGORITM
		List<Integer> listaIntoarsa = c1.runAlgorithm("./output"+ fileNumber +".arff");
