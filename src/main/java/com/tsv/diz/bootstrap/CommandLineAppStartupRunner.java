...
public class CommandLineAppStartupRunner implements CommandLineRunner{
	...
	@Autowired
	private Cluster c1;
	private List<String> listaRezultate = new ArrayList<>();
	private List<Integer> listaRezultateId = new ArrayList<>();
	private int count = 1;
	private int max = 1;
	...
	public void run(String... args) throws Exception {
		...
		for(String element: listaRezultate) {
			System.out.println("Vectorul contine: " + element);
		}
       ...
       resultRepo.saveAll(rezultate);
        ...
        //creez lista rezultate
        //adaug in lista aia rezultatul option_Id si user_id
	}
		

		

		List<OptionQuery> optiuni = optionRepo.getOptions();
		for(int i=0; i<optiuni.size(); i++) {
			System.out.println(optiuni.get(i).getUser_id());
			System.out.println(optiuni.get(i).getOptionUser());
		}
	}
